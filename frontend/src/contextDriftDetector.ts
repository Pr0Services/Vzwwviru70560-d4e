/* =====================================================
   CHEÂ·NU â€” CONTEXT DRIFT DETECTOR
...
export class ContextDriftDetectorAgent {
  ...
  private analyzeContextType(
    ...
    for (const record of recentOfType) {
      if (record.transitionFrom) {
        const key = `${record.transitionFrom}â†’${contextType}`;
    ...
    const commonTransitions = [...transitionCounts.entries()]
      ...
      .slice(0, 3)
      .map(([key, freq]) => {
        const [from, to] = key.split('â†’') as [ContextType, ContextType];
  ...
  private analyzeTransitions(
    ...
    for (const record of historical) {
      if (record.transitionFrom) {
        const key = `${record.transitionFrom}â†’${record.contextType}`;
...
You are the CHEÂ·NU Context Drift Detector.
...
Common Transitions:
${report.observedMetrics.commonTransitions.map((t) => `  ${t.from} â†’ ${t.to}: ${t.frequency}`).join('\n')}
` : `
...
âš ï¸ This report is OBSERVATIONAL ONLY.
