/**
 * CHEÂ·NU V5.0 - SAFETY MANAGER (Gestion SST/CNESST)
 */
...
const mockSites = [
  { id: 's1', name: 'RÃ©sidence Tremblay', address: 'MontrÃ©al', riskLevel: 'medium' },
...
const mockInspections: Inspection[] = [
  { id: 'insp-1', date: '2024-12-05', siteId: 's1', inspector: 'Jean Gagnon', type: 'routine', status: 'passed', score: 92, items: [
    ...
    { category: 'Ã‰chafaudages', item: 'StabilitÃ©', status: 'warning', notes: 'Ã€ resserrer' },
    { category: 'Ã‰lectricitÃ©', item: 'CÃ¢blage', status: 'ok', notes: '' },
  ]},
  { id: 'insp-2', date: '2024-12-03', siteId: 's2', inspector: 'Marc Dubois', type: 'cnesst', status: 'failed', score: 68, items: [
    ...
    { category: 'EPI', item: 'Harnais', status: 'warning', notes: '2 expirÃ©s' },
  ]},
];

const mockIncidents: Incident[] = [
  { id: 'inc-1', date: '2024-12-04', siteId: 's1', type: 'near-miss', severity: 'medium', description: 'Chute d\'outil du 3e Ã©tage - aucun blessÃ©', rootCause: 'Mauvaise fixation', correctiveActions: ['Formation rappel', 'Filets de protection'], reportedToCnesst: false, status: 'closed' },
  { id: 'inc-2', date: '2024-12-02', siteId: 's2', type: 'first-aid', severity: 'low', description: 'Coupure mineure Ã  la main', injuredWorker: 'Pierre Roy', rootCause: 'Gants non portÃ©s', correctiveActions: ['Rappel EPI obligatoire'], reportedToCnesst: false, status: 'closed' },
];
...
const StatusBadge = ({ status, type }: { status: string; type: 'inspection' | 'incident' | 'training' }) => {
  const configs: Record<string, Record<string, { label: string; color: string }>> = {
    inspection: { passed: { label: 'âœ… RÃ©ussi', color: colors.emerald }, failed: { label: 'âŒ Ã‰chouÃ©', color: colors.red }, pending: { label: 'â³ En cours', color: colors.orange } },
    incident: { open: { label: 'ðŸ”´ Ouvert', color: colors.red }, investigating: { label: 'ðŸ” EnquÃªte', color: colors.orange }, closed: { label: 'âœ… FermÃ©', color: colors.emerald } },
    training: { valid: { label: 'âœ… Valide', color: colors.emerald }, expiring: { label: 'âš ï¸ Expire bientÃ´t', color: colors.orange }, expired: { label: 'âŒ ExpirÃ©', color: colors.red } },
  };
...
const SeverityBadge = ({ severity }: { severity: string }) => {
  const cfg: Record<string, { label: string; color: string }> = {
    ...
    high: { label: 'Ã‰levÃ©', color: colors.orange },
...
const SafetyManager: React.FC = () => {
  ...
  const tabs = [
    ...
    { id: 'inspections', label: 'ðŸ” Inspections', icon: 'ðŸ”' },
    { id: 'incidents', label: 'âš ï¸ Incidents', icon: 'âš ï¸' },
    { id: 'training', label: 'ðŸ“š Formations', icon: 'ðŸ“š' },
  ];
  ...
  return (
    <div style={{ minHeight: '100vh', background: colors.dark, color: colors.sand, fontFamily: 'Inter' }}>
      ...
      <div style={{ padding: '20px 32px', borderBottom: `1px solid ${colors.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 700, margin: 0 }}>ðŸ¦º Gestion SST</h1>
          <p style={{ color: colors.stone, margin: '4px 0 0', fontSize: 13 }}>SÃ©curitÃ©, inspections et conformitÃ© CNESST</p>
        </div>
        <div style={{ display: 'flex', gap: 12 }}>
          <button style={{ padding: '10px 20px', background: colors.red, border: 'none', borderRadius: 10, color: '#fff', fontSize: 14, cursor: 'pointer' }}>ðŸš¨ DÃ©clarer incident</button>
      ...
      <div style={{ padding: 32 }}>
        ...
        {activeTab === 'dashboard' && (
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20, marginBottom: 32 }}>
              <StatCard icon="ðŸ›¡ï¸" value={stats.daysWithoutIncident} label="Jours sans incident" color={colors.emerald} />
              <StatCard icon="ðŸ”" value={stats.inspectionsThisMonth} label="Inspections ce mois" color={colors.turquoise} />
              <StatCard icon="âœ…" value={`${stats.passRate}%`} label="Taux de conformitÃ©" color={stats.passRate >= 80 ? colors.emerald : colors.orange} />
              <StatCard icon="âš ï¸" value={stats.openIncidents} label="Incidents ouverts" color={stats.openIncidents > 0 ? colors.red : colors.emerald} />
            </div>
            ...
            {(stats.expiringCerts > 0 || stats.expiredCerts > 0) && (
              <div style={{ background: `${colors.orange}15`, border: `1px solid ${colors.orange}40`, borderRadius: 12, padding: 20, marginBottom: 24 }}>
                <h3 style={{ margin: '0 0 12px', fontSize: 16, color: colors.orange }}>âš ï¸ Alertes de formation</h3>
                <div style={{ display: 'flex', gap: 24 }}>
                  {stats.expiredCerts > 0 && <div style={{ color: colors.red }}>âŒ {stats.expiredCerts} certification(s) expirÃ©e(s)</div>}
                  {stats.expiringCerts > 0 && <div style={{ color: colors.orange }}>â° {stats.expiringCerts} certification(s) expire(nt) bientÃ´t</div>}
                </div>
              </div>
            )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
              <div style={{ background: colors.card, border: `1px solid ${colors.border}`, borderRadius: 12, padding: 20 }}>
                <h3 style={{ margin: '0 0 16px', fontSize: 16 }}>ðŸ” DerniÃ¨res inspections</h3>
                {inspections.slice(0, 3).map(insp => (
                  <div key={insp.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: `1px solid ${colors.border}` }}>
                    <div>
                      ...
                      <div style={{ color: colors.stone, fontSize: 12 }}>{insp.date} â€¢ {insp.inspector}</div>
                    </div>
              ...
              <div style={{ background: colors.card, border: `1px solid ${colors.border}`, borderRadius: 12, padding: 20 }}>
                <h3 style={{ margin: '0 0 16px', fontSize: 16 }}>âš ï¸ Incidents rÃ©cents</h3>
                {incidents.slice(0, 3).map(inc => (
                  <div key={inc.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: `1px solid ${colors.border}` }}>
                    <div>
                      ...
                      <div style={{ color: colors.stone, fontSize: 12 }}>{inc.date} â€¢ {mockSites.find(s => s.id === inc.siteId)?.name}</div>
                    </div>
        ...
        {activeTab === 'inspections' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24 }}>
              <h2 style={{ margin: 0 }}>ðŸ” Inspections</h2>
              <input type="text" placeholder="ðŸ” Rechercher..." style={{ padding: '10px 16px', background: colors.slate, border: `1px solid ${colors.border}`, borderRadius: 8, color: colors.sand, width: 250 }} />
            </div>
                ...
                <tbody>
                  {inspections.map(insp => (
                    <tr key={insp.id} style={{ borderBottom: `1px solid ${colors.border}` }}>
                      ...
                      <td style={{ padding: 14 }}><button style={{ background: 'none', border: 'none', color: colors.gold, cursor: 'pointer' }}>Voir â†’</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {activeTab === 'incidents' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24 }}>
              <h2 style={{ margin: 0 }}>âš ï¸ Incidents</h2>
              <button style={{ padding: '10px 20px', background: colors.red, border: 'none', borderRadius: 8, color: '#fff', cursor: 'pointer' }}>+ DÃ©clarer incident</button>
            </div>
            {incidents.map(inc => (
              <div key={inc.id} style={{ background: colors.card, border: `1px solid ${colors.border}`, borderRadius: 12, padding: 20, marginBottom: 16 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <span style={{ fontSize: 24 }}>{ inc.type === 'near-miss' ? 'ðŸ‘ï¸' : inc.type === 'first-aid' ? 'ðŸ©¹' : 'ðŸš‘' }</span>
                    <div>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <SeverityBadge severity={inc.severity} />
                    <StatusBadge status={inc.status} type="incident" />
                    {inc.reportedToCnesst && <span style={{ padding: '4px 10px', background: `${colors.blue}20`, color: colors.blue, borderRadius: 20, fontSize: 11 }}>ðŸ“‹ CNESST</span>}
                  </div>
                </div>
                <p style={{ margin: '0 0 12px', color: colors.sand }}>{inc.description}</p>
                {inc.injuredWorker && <div style={{ color: colors.orange, fontSize: 13, marginBottom: 8 }}>ðŸ‘¤ Travailleur: {inc.injuredWorker}</div>}
                ...
                {inc.correctiveActions.length > 0 && (
                  <div style={{ marginTop: 12 }}>
                    ...
                    {inc.correctiveActions.map((action, i) => (
                      <span key={i} style={{ display: 'inline-block', padding: '4px 10px', background: colors.slate, borderRadius: 6, fontSize: 12, marginRight: 8, marginBottom: 4, color: colors.sand }}>âœ“ {action}</span>
                    ))}
                  </div>
                )}
              </div>
        ...
        {activeTab === 'training' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24 }}>
              <h2 style={{ margin: 0 }}>ðŸ“š Formations & Certifications</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }}>
              ...
              <div style={{ background: `${colors.orange}15`, border: `1px solid ${colors.orange}40`, borderRadius: 12, padding: 16, textAlign: 'center' }}>
                ...
                <div style={{ color: colors.stone, fontSize: 12 }}>Expirent bientÃ´t</div>
              </div>
              <div style={{ background: `${colors.red}15`, border: `1px solid ${colors.red}40`, borderRadius: 12, padding: 16, textAlign: 'center' }}>
                ...
                <div style={{ color: colors.stone, fontSize: 12 }}>ExpirÃ©es</div>
              </div>
            </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: colors.slate }}>
                    ...
                    <th style={{ padding: 14, textAlign: 'left', color: colors.stone, fontSize: 12 }}>Ã‰MISSION</th>
                    ...
                    <tr key={t.id} style={{ borderBottom: `1px solid ${colors.border}` }}>
                      <td style={{ padding: 14 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          <div style={{ width: 32, height: 32, background: `linear-gradient(135deg, ${colors.emerald}, ${colors.moss})`, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12 }}>ðŸ‘·</div>
