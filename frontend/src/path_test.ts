/* =========================================
   CHEÂ·NU â€” PATH SYSTEM TESTS
   
   Tests du systÃ¨me de chemins.
   VÃ©rifie le respect du manifeste directionnel.
   ========================================= */
...
describe('CHEÂ·NU Path System', () => {
  ...
  describe('Chemin B â€” Nouvel Objectif', () => {
  ...
  describe('Chemin C â€” Exploration', () => {
    const path: PathType = 'exploration';

    it('should have correct intention', () => {
      expect(PATHS[path].intention).toBe('Je rÃ©flÃ©chis / je dÃ©couvre');
    });
  ...
  describe('Intention Parsing', () => {
    it('should parse reprise intentions', () => {
      ...
      expect(parseIntention('Je reprends oÃ¹ j\'Ã©tais')).toBe('reprise');
    });

    it('should parse objectif intentions', () => {
      expect(parseIntention('Je commence un nouveau projet')).toBe('objectif');
      expect(parseIntention('Je veux crÃ©er quelque chose')).toBe('objectif');
    });

    it('should parse exploration intentions', () => {
      expect(parseIntention('Je veux explorer les options')).toBe('exploration');
      expect(parseIntention('Je rÃ©flÃ©chis Ã  cette idÃ©e')).toBe('exploration');
    });

    it('should parse decision intentions', () => {
      expect(parseIntention('Je dois dÃ©cider maintenant')).toBe('decision');
  ...
  describe('Path Engine', () => {
    ...
    it('should enter a path with intention', () => {
      ...
      expect(getCurrentIntention()).toBe('Je rÃ©flÃ©chis / je dÃ©couvre');
    });
