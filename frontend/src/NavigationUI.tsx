/* =====================================================
   CHEÂ·NU â€” Navigation UI Components
...
export const Breadcrumb = memo(function Breadcrumb({ maxVisible = 4, showIcons = true, separator = 'â€º', className = '' }: BreadcrumbProps) {
  ...
  const icons: Record<string, string> = { universe: 'ðŸŒŒ', sphere: 'ðŸ”®', branch: 'ðŸŒ¿', leaf: 'ðŸƒ', 'agent-chat': 'ðŸ¤–' };
          ...
          <li key={item.id} className={`breadcrumb__item ${item.isActive ? 'breadcrumb__item--active' : ''}`}>
            ...
            {item.type === 'ellipsis' ? <span className="breadcrumb__ellipsis">...</span> : (
              <button className="breadcrumb__link" onClick={item.onClick} disabled={item.isActive} aria-current={item.isActive ? 'page' : undefined}>
                {showIcons && <span className="breadcrumb__icon" aria-hidden="true">{icons[item.type] || 'ðŸ“'}</span>}
    ...
    <header className={`navigation-bar ${className}`}>
      {showBackForward && (
        <div className="navigation-bar__controls">
          <button className="navigation-bar__button" onClick={goBack} disabled={!canGoBack} aria-label="Go back" title="Go back (â†)">â†</button>
          <button className="navigation-bar__button" onClick={goForward} disabled={!canGoForward} aria-label="Go forward" title="Go forward (â†’)">â†’</button>
        </div>
      )}
      {showHomeButton && <button className="navigation-bar__button navigation-bar__home" onClick={goToRoot} disabled={isAtRoot()} aria-label="Go to universe" title="Go to universe (Home)">ðŸ </button>}
...
export const BackButton = memo(function BackButton({ label = 'Back', showLabel = true, className = '' }: BackButtonProps) {
  ...
  return (
    <button className={`back-button ${className}`} onClick={goBack} aria-label={label}>
      <span className="back-button__icon">â†</span>
