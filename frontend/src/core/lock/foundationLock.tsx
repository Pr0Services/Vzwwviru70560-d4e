...
export class FoundationLock {
  ...
  halt(reason: string): never {
    logger.error('╔═══════════════════════════════════════════════════════╗');
    logger.error('║         ⛔ FOUNDATION INTEGRITY FAILURE               ║');
    logger.error('╠═══════════════════════════════════════════════════════╣');
    logger.error(`║  Reason: ${reason.padEnd(45)}║`);
    logger.error('║                                                       ║');
    logger.error('║  The system cannot proceed without a valid foundation.║');
    logger.error('║  No agent, system, or optimization may override this. ║');
    logger.error('║                                                       ║');
    logger.error('╚═══════════════════════════════════════════════════════╝');
  ...
  async startSystem(
    ...
    if (state.status === 'LOCKED') {
      logger.debug('✅ Foundation verified. System starting...');
      onSuccess();
    } else {
...
❤️ CHE·NU — Governed Intelligence Operating System
`;
