/* =========================================================
   CHEÂ·NU â€” Process Analysis (Adaptation Layer)
    ...
    detect: (actions) => {
      ...
      if (rollbacks.length > 3) {
        return {
          ...
          stage: 'navigation',
          note: `Multiple rollbacks detected (${rollbacks.length}) â€” user may be exploring or uncertain.`,
          severity: 'info',
        };
      }
      return null;
    ...
    detect: (actions) => {
      ...
      if (recent.length > 5) {
        return {
          timestamp: new Date().toISOString(),
          stage: 'navigation',
          note: 'Rapid sphere switching detected â€” user may be searching for something.',
          severity: 'info',
        };
      }
      return null;
    ...
    detect: (actions) => {
      ...
      if (sessionDuration > 30 * 60 * 1000 && !hasDecisions) { // 30 minutes
        return {
          ...
          note: 'Long session without decisions â€” user may be in exploration mode.',
          severity: 'info',
        };
      }
      return null;
    ...
    detect: (actions) => {
      ...
      if (loops > 3) {
        return {
          timestamp: new Date().toISOString(),
          stage: 'navigation',
          note: `Navigation loops detected (${loops}) â€” user may benefit from clarity.`,
          severity: 'concern',
        };
      }
      return null;
    ...
    detect: (actions) => {
      if (actions.some((a) => a.type === 'isolation_breach_blocked')) {
        return {
          timestamp: new Date().toISOString(),
          stage: 'parallel_analysis',
          note: 'Agent attempted to access another agent output â€” blocked by guards.',
          severity: 'warning',
        };
      }
      return null;
...
export function analyzeNavigationPatterns(
  ...
  return {
    ...
    suggestion:
      loops > 3
        ? 'User may be searching â€” consider offering navigation assistance.'
        : undefined,
  };
}
 ...
 * 
 * âš ï¸ This is OBSERVATIONAL only â€” no decisions or optimizations.
 */
