...
const categoryColors: Record<string, string> = { 'RÃ©union': colors.blue, 'Chantier': colors.emerald, 'Livraison': colors.orange, 'Inspection': colors.purple, 'Personnel': colors.turquoise, 'Autre': colors.stone };

const mockEvents: Event[] = [
  { id: 'e1', title: 'RÃ©union Ã©quipe', date: '2024-12-05', startTime: '09:00', endTime: '10:00', category: 'RÃ©union', location: 'Bureau', attendees: ['Jean', 'Marc'] },
  ...
  { id: 'e3', title: 'Livraison matÃ©riaux', date: '2024-12-06', startTime: '08:00', endTime: '09:00', category: 'Livraison', description: 'BÃ©ton + acier' },
  { id: 'e4', title: 'Inspection RBQ', date: '2024-12-06', startTime: '13:00', endTime: '15:00', category: 'Inspection', location: 'Chantier Plaza' },
  { id: 'e5', title: 'Rendez-vous client', date: '2024-12-07', startTime: '10:00', endTime: '11:30', category: 'RÃ©union', attendees: ['Sophie Tremblay'] },
  { id: 'e6', title: 'Formation sÃ©curitÃ©', date: '2024-12-09', startTime: '08:00', endTime: '12:00', category: 'Autre', location: 'Centre CNESST' },
  ...
  return (
    <div style={{ minHeight: '100vh', background: colors.dark, color: colors.sand, fontFamily: 'Inter' }}>
      <div style={{ padding: '20px 32px', borderBottom: `1px solid ${colors.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div><h1 style={{ fontSize: 24, margin: 0 }}>ðŸ“… Calendrier</h1></div>
        <div style={{ display: 'flex', gap: 12 }}>
          ...
          <button onClick={() => setSelectedEvent({ id: '', title: '', date: selectedDate || formatDate(new Date().getDate()), startTime: '09:00', endTime: '10:00', category: 'Autre' })} style={{ padding: '10px 20px', background: colors.gold, border: 'none', borderRadius: 10, color: colors.dark, fontWeight: 600, cursor: 'pointer' }}>+ Ã‰vÃ©nement</button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', height: 'calc(100vh - 80px)' }}>
        ...
        <div style={{ padding: 24 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              <button onClick={prevMonth} style={{ background: colors.slate, border: 'none', borderRadius: 8, padding: '8px 12px', color: colors.sand, cursor: 'pointer' }}>â†</button>
              <h2 style={{ margin: 0, fontSize: 20, minWidth: 200, textAlign: 'center' }}>{months[month]} {year}</h2>
              <button onClick={nextMonth} style={{ background: colors.slate, border: 'none', borderRadius: 8, padding: '8px 12px', color: colors.sand, cursor: 'pointer' }}>â†’</button>
            </div>
        ...
        <div style={{ borderLeft: `1px solid ${colors.border}`, padding: 20, overflow: 'auto' }}>
          <h3 style={{ margin: '0 0 16px', fontSize: 16 }}>
            {selectedDate ? new Date(selectedDate + 'T12:00:00').toLocaleDateString('fr-CA', { weekday: 'long', day: 'numeric', month: 'long' }) : 'SÃ©lectionnez un jour'}
          </h3>
              ...
              {selectedDateEvents.length === 0 ? (
                ...
                selectedDateEvents.map(e => (
                  <div key={e.id} onClick={() => setSelectedEvent(e)} style={{
                    ...
                    <div style={{ color: colors.turquoise, fontSize: 13, marginBottom: 4 }}>ðŸ• {e.startTime} - {e.endTime}</div>
                    {e.location && <div style={{ color: colors.stone, fontSize: 12 }}>ðŸ“ {e.location}</div>}
                    {e.attendees && e.attendees.length > 0 && <div style={{ color: colors.stone, fontSize: 12, marginTop: 4 }}>ðŸ‘¥ {e.attendees.join(', ')}</div>}
                  </div>
                ))
              )}

              {/* Quick Add */}
              <button style={{ width: '100%', padding: 12, background: 'transparent', border: `1px dashed ${colors.border}`, borderRadius: 10, color: colors.stone, cursor: 'pointer', marginTop: 12 }}>+ Ajouter Ã©vÃ©nement</button>
            </>
          )}

          {/* Categories Legend */}
          <div style={{ marginTop: 24, paddingTop: 24, borderTop: `1px solid ${colors.border}` }}>
            <h4 style={{ color: colors.stone, fontSize: 12, margin: '0 0 12px' }}>CATÃ‰GORIES</h4>
          ...
          {/* Upcoming Events */}
          <div style={{ marginTop: 24, paddingTop: 24, borderTop: `1px solid ${colors.border}` }}>
            <h4 style={{ color: colors.stone, fontSize: 12, margin: '0 0 12px' }}>Ã€ VENIR</h4>
            {events.slice(0, 4).map(e => (
              <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: `1px solid ${colors.border}` }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 11, color: colors.stone }}>{e.date} â€¢ {e.startTime}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setSelectedEvent(null)}>
          <div style={{ background: colors.dark, border: `1px solid ${colors.border}`, borderRadius: 16, width: 450, padding: 24 }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
              ...
              <button onClick={() => setSelectedEvent(null)} style={{ background: 'none', border: 'none', color: colors.stone, fontSize: 20, cursor: 'pointer' }}>âœ•</button>
            </div>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
              <button style={{ flex: 1, padding: 12, background: colors.slate, border: `1px solid ${colors.border}`, borderRadius: 8, color: colors.sand, cursor: 'pointer' }}>âœï¸ Modifier</button>
              <button style={{ flex: 1, padding: 12, background: `${colors.red}20`, border: `1px solid ${colors.red}`, borderRadius: 8, color: colors.red, cursor: 'pointer' }}>ðŸ—‘ï¸ Supprimer</button>
            </div>
          </div>
        </div>
      )}
