/**
 * CHEÂ·NU V5.0 - QUOTE BUILDER (GÃ©nÃ©rateur de soumissions)
 */
...
const mockQuotes: Quote[] = [
  { id: 'q1', number: 'SOU-2024-001', clientName: 'Martin Lavoie', clientEmail: 'martin@email.com', projectName: 'RÃ©novation cuisine', projectAddress: '123 Rue Exemple, MontrÃ©al', items: [
    { id: 'i1', description: 'DÃ©molition cuisine existante', quantity: 1, unit: 'forfait', unitPrice: 2500, category: 'DÃ©molition' },
    { id: 'i2', description: 'Armoires sur mesure', quantity: 12, unit: 'pi lin.', unitPrice: 450, category: 'Menuiserie' },
    { id: 'i3', description: 'Comptoir quartz', quantity: 25, unit: 'piÂ²', unitPrice: 85, category: 'Finition' },
    { id: 'i4', description: 'Installation plomberie', quantity: 1, unit: 'forfait', unitPrice: 3200, category: 'Plomberie' },
    { id: 'i5', description: 'Installation Ã©lectrique', quantity: 1, unit: 'forfait', unitPrice: 2800, category: 'Ã‰lectricitÃ©' },
  ], validUntil: '2024-12-31', status: 'sent', notes: 'DÃ©lai estimÃ©: 4-6 semaines', createdAt: '2024-12-01', margin: 25 },
  { id: 'q2', number: 'SOU-2024-002', clientName: 'Sophie Tremblay', clientEmail: 'sophie@email.com', projectName: 'Salle de bain principale', projectAddress: '456 Ave des Pins, Laval', items: [
    { id: 'i6', description: 'DÃ©molition salle de bain', quantity: 1, unit: 'forfait', unitPrice: 1800, category: 'DÃ©molition' },
    { id: 'i7', description: 'CÃ©ramique plancher + murs', quantity: 180, unit: 'piÂ²', unitPrice: 12, category: 'Finition' },
    { id: 'i8', description: 'VanitÃ© double', quantity: 1, unit: 'unitÃ©', unitPrice: 2200, category: 'Mobilier' },
  ], validUntil: '2024-12-20', status: 'accepted', notes: '', createdAt: '2024-11-28', margin: 30 },
  { id: 'q3', number: 'SOU-2024-003', clientName: 'Jean-Pierre Roy', clientEmail: 'jp.roy@email.com', projectName: 'Terrasse extÃ©rieure', projectAddress: '789 Boul. du Parc, Longueuil', items: [
    { id: 'i9', description: 'Structure terrasse bois traitÃ©', quantity: 200, unit: 'piÂ²', unitPrice: 35, category: 'Structure' },
...
const itemLibrary = [
  { desc: 'DÃ©molition gÃ©nÃ©rale', unit: 'forfait', price: 2000, cat: 'DÃ©molition' },
  { desc: 'Main d\'oeuvre gÃ©nÃ©rale', unit: 'heure', price: 65, cat: 'Main d\'oeuvre' },
  { desc: 'Gypse 1/2"', unit: 'feuille', price: 18, cat: 'MatÃ©riaux' },
  { desc: 'Peinture intÃ©rieure', unit: 'piÂ²', price: 4, cat: 'Finition' },
  { desc: 'CÃ©ramique pose', unit: 'piÂ²', price: 8, cat: 'Finition' },
  { desc: 'Ã‰lectricien', unit: 'heure', price: 85, cat: 'Ã‰lectricitÃ©' },
  { desc: 'Plombier', unit: 'heure', price: 90, cat: 'Plomberie' },
];

const StatusBadge = ({ status }: { status: Quote['status'] }) => {
  const cfg: Record<string, { l: string; c: string }> = { draft: { l: 'ðŸ“ Brouillon', c: colors.stone }, sent: { l: 'ðŸ“¤ EnvoyÃ©e', c: colors.blue }, accepted: { l: 'âœ… AcceptÃ©e', c: colors.emerald }, rejected: { l: 'âŒ RefusÃ©e', c: colors.red }, expired: { l: 'â° ExpirÃ©e', c: colors.orange } };
  const c = cfg[status];
...
const QuoteBuilder: React.FC = () => {
  ...
  const addItem = () => {
    if (!editing) return;
    setEditing({ ...editing, items: [...editing.items, { id: `i-${Date.now()}`, description: '', quantity: 1, unit: 'unitÃ©', unitPrice: 0, category: 'GÃ©nÃ©ral' }] });
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: colors.dark, color: colors.sand, fontFamily: 'Inter' }}>
      <div style={{ padding: '20px 32px', borderBottom: `1px solid ${colors.border}`, display: 'flex', justifyContent: 'space-between' }}>
        <div><h1 style={{ fontSize: 24, margin: 0 }}>ðŸ“‘ Soumissions</h1><p style={{ color: colors.stone, margin: '4px 0 0', fontSize: 13 }}>{stats.total} soumissions â€¢ {formatC(stats.totalValue)} acceptÃ©es</p></div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '350px 1fr', height: 'calc(100vh - 80px)' }}>
          <div style={{ borderRight: `1px solid ${colors.border}`, padding: 20, overflow: 'auto' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
              <div style={{ background: colors.card, border: `1px solid ${colors.border}`, borderRadius: 12, padding: 16, textAlign: 'center' }}><div style={{ color: colors.emerald, fontSize: 20, fontWeight: 700 }}>{stats.accepted}</div><div style={{ color: colors.stone, fontSize: 11 }}>AcceptÃ©es</div></div>
            ...
            <select value={filter} onChange={e => setFilter(e.target.value)} style={{ width: '100%', padding: '10px 14px', background: colors.slate, border: `1px solid ${colors.border}`, borderRadius: 8, color: colors.sand, marginBottom: 16 }}>
              <option value="all">Toutes ({stats.total})</option><option value="draft">Brouillons</option><option value="sent">EnvoyÃ©es</option><option value="accepted">AcceptÃ©es</option><option value="rejected">RefusÃ©es</option>
            </select>
            {filtered.map(q => {
              ...
              return (
                <div key={q.id} onClick={() => setSelected(q)} style={{ padding: 16, background: selected?.id === q.id ? `${colors.gold}10` : colors.card, border: `1px solid ${selected?.id === q.id ? colors.gold : colors.border}`, borderRadius: 12, marginBottom: 10, cursor: 'pointer' }}>
                  ...
                  <div style={{ color: colors.stone, fontSize: 13 }}>{q.clientName || 'Client non dÃ©fini'}</div>
          ...
          <div style={{ padding: 24, overflow: 'auto' }}>
                ...
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
                    ...
                    <div style={{ display: 'flex', gap: 12 }}>
                      <button onClick={() => { setEditing(selected); setView('edit'); }} style={{ padding: '10px 20px', background: colors.slate, border: `1px solid ${colors.border}`, borderRadius: 8, color: colors.sand, cursor: 'pointer' }}>âœï¸ Modifier</button>
                      <button style={{ padding: '10px 20px', background: colors.moss, border: 'none', borderRadius: 8, color: '#fff', cursor: 'pointer' }}>ðŸ“„ PDF</button>
                      {selected.status === 'draft' && <button style={{ padding: '10px 20px', background: colors.turquoise, border: 'none', borderRadius: 8, color: '#fff', cursor: 'pointer' }}>ðŸ“¤ Envoyer</button>}
                    </div>
                  </div>
                  <div style={{ background: '#fff', color: '#1a1a1a', padding: 40, borderRadius: 8 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 30 }}>
                      ...
                      <div style={{ textAlign: 'right', fontSize: 13 }}><strong>Services Pro Construction</strong><br />4567 Rue du Commerce, MontrÃ©al<br />514-555-0000</div>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 20 }}>
                      <thead><tr style={{ borderBottom: '2px solid #ddd' }}><th style={{ textAlign: 'left', padding: 8, fontSize: 11, color: '#888' }}>DESCRIPTION</th><th style={{ textAlign: 'center', padding: 8, fontSize: 11, color: '#888' }}>QTÃ‰</th><th style={{ textAlign: 'center', padding: 8, fontSize: 11, color: '#888' }}>UNITÃ‰</th><th style={{ textAlign: 'right', padding: 8, fontSize: 11, color: '#888' }}>PRIX</th><th style={{ textAlign: 'right', padding: 8, fontSize: 11, color: '#888' }}>TOTAL</th></tr></thead>
            ...
            })() : <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', color: colors.stone }}><span style={{ fontSize: 48, marginBottom: 16 }}>ðŸ“‘</span>SÃ©lectionnez une soumission</div>}
          </div>
        </div>
      )}
      ...
      {view === 'edit' && editing && (
        <div style={{ padding: 32 }}>
          ...
          <div style={{ background: colors.card, border: `1px solid ${colors.border}`, borderRadius: 12, padding: 20, marginBottom: 24 }}>
            <h3 style={{ margin: '0 0 16px' }}>ðŸ“¦ Articles</h3>
            {editing.items.map((item, idx) => (
              <div key={item.id} style={{ display: 'grid', gridTemplateColumns: '2fr 80px 80px 100px 100px 40px', gap: 10, marginBottom: 10 }}>
                ...
                <button onClick={() => removeItem(idx)} style={{ background: 'none', border: 'none', color: colors.red, cursor: 'pointer', fontSize: 18 }}>âœ•</button>
              </div>
            ))}
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <div style={{ width: 300, background: colors.card, border: `1px solid ${colors.border}`, borderRadius: 12, padding: 20 }}>
              {(() => { const t = calcTotals(editing.items, editing.margin); return <>
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0' }}><span>CoÃ»t</span><span>{formatC(t.cost)}</span></div>
