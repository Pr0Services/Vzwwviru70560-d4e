  ...
  devtools(
    (set, get) => ({
      ...
      getGuidance: async (request) => {
        ...
        try {
          const response = await novaConstitutionService.getGuidance(request);
          set({ isProcessing: false });
          return response;
        } catch (error: unknown) {
          set({
            isProcessing: false,
            error: error.message || 'Failed to get guidance',
          });
          return null;
        }
      },
      ...
      requestDeepen: async (analysisId) => {
        ...
        try {
          ...
          if ('checkpoint' in response && response.checkpoint) {
            ...
            const { default: useGovernanceStore } = await import('./governanceStore');
            useGovernanceStore.getState().setActiveCheckpoint(response.checkpoint as any);
          }
          ...
          set({
            isProcessing: false,
            error: error.message || 'Failed to deepen analysis',
          });
        }
      },
