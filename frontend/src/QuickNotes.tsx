...
const NoteCard = ({ note, isSelected, onClick }: { note: Note; isSelected: boolean; onClick: () => void }) => (
  <div onClick={onClick} style={{
    ...
    {note.pinned && <span style={{ position: 'absolute', top: 8, right: 8, fontSize: 14 }}>ðŸ“Œ</span>}
    ...
    <div style={{ minHeight: '100vh', background: colors.dark, color: colors.sand, fontFamily: 'Inter', display: 'flex' }}>
      ...
      <div style={{ width: 220, borderRight: `1px solid ${colors.border}`, padding: 20 }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="ðŸ” Rechercher..." style={{ width: '100%', padding: '10px 14px', background: colors.slate, border: `1px solid ${colors.border}`, borderRadius: 8, color: colors.sand, fontSize: 13 }} />
        </div>
      ...
      <div style={{ width: 320, borderRight: `1px solid ${colors.border}`, padding: 20, overflow: 'auto' }}>
        <h3 style={{ margin: '0 0 16px', fontSize: 16 }}>ðŸ“ {selectedFolder} ({filteredNotes.length})</h3>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', gap: 12, marginTop: 8, color: colors.stone, fontSize: 13 }}>
                  <span>ðŸ“ {selectedNote.folder}</span>
                  <span>ðŸ“… {selectedNote.updatedAt}</span>
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={handleTogglePin} style={{ padding: '8px 12px', background: selectedNote.pinned ? colors.gold : colors.slate, border: `1px solid ${colors.border}`, borderRadius: 8, color: selectedNote.pinned ? colors.dark : colors.sand, cursor: 'pointer' }}>ðŸ“Œ</button>
                {editMode ? (
                  <button onClick={handleSave} style={{ padding: '8px 16px', background: colors.emerald, border: 'none', borderRadius: 8, color: '#fff', cursor: 'pointer' }}>ðŸ’¾ Sauvegarder</button>
                ) : (
                  <button onClick={() => { setEditMode(true); setEditContent(selectedNote.content); }} style={{ padding: '8px 16px', background: colors.slate, border: `1px solid ${colors.border}`, borderRadius: 8, color: colors.sand, cursor: 'pointer' }}>âœï¸ Modifier</button>
                )}
                <button onClick={handleDelete} style={{ padding: '8px 12px', background: `${colors.red}20`, border: `1px solid ${colors.red}`, borderRadius: 8, color: colors.red, cursor: 'pointer' }}>ðŸ—‘ï¸</button>
              </div>
            </div>
