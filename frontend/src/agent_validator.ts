/* =========================================
   CHEÂ·NU â€” AGENT VALIDATOR
   
   Validateur qui enforce les rÃ¨gles du manifeste.
...
function logViolation(violation: AgentViolation): void {
  ...
  if (violation.severity === 'critical') {
    validatorLogger.error(`ðŸš¨ CRITICAL VIOLATION: ${violation.message}`, violation);
  } else {
    validatorLogger.warn(`âš ï¸ Violation: ${violation.message}`, violation);
  }
}
...
/**
 * Valider une tentative d'Ã©criture timeline
 */
...
export function validateAgentOutput(output: AgentOutput): AgentViolation[] {
  ...
  // 1. VÃ©rifier que l'explication est prÃ©sente
...
export function validateSuggestion(
  ...
  // VÃ©rifier les prÃ©fixes interdits (langage impÃ©ratif)
  ...
  // VÃ©rifier que le prÃ©fixe est valide
...
/**
 * Guard pour empÃªcher les actions interdites
 */
export function guardAgentAction<T>(
  ...
  if (violation) {
    validatorLogger.error('ðŸ›‘ Agent action blocked', violation);
    return null;
  }
  return fn();
}
...
/**
 * Guard pour empÃªcher les Ã©critures timeline
 */
export function guardTimelineWrite<T>(
  ...
  if (violation) {
    validatorLogger.error('ðŸ›‘ Timeline write blocked', violation);
    return null;
  }
  return fn();
}
...
export function guardCommunication<T>(
  ...
  if (violation) {
    validatorLogger.error('ðŸ›‘ Communication blocked', violation);
    return null;
  }
  return fn();
}
...
export function guardAgentOutput<T extends AgentOutput>(
  ...
  if (violations.some((v) => v.severity === 'critical')) {
    validatorLogger.error('ðŸ›‘ Agent output blocked due to critical violation');
    return null;
  }
...
export function generateComplianceReport(): AgentComplianceReport {
  ...
  return {
    ...
    summary: criticalCount === 0
      ? `âœ… Agent system compliant. ${violations.length} minor violation(s).`
      : `ðŸš¨ ${criticalCount} critical violation(s) detected!`,
  };
}
